...
def R1_Semiint(I,t):
    ...
    >>> from scipy.integrate import cumulative_trapezoid
    >>> df1 = pd.read_csv('TestFiles/Testfile_1.csv')
    >>> np.allclose(R1_Semiint(R1_Semiint(df1['I'],df1['t']), df1['t'])[:-1], cumulative_trapezoid(df1['I'],df1['t']), rtol=1e-0)
    True
    >>> df2 = pd.read_csv('TestFiles/Testfile_2.csv')
    >>> np.allclose(R1_Semiint(R1_Semiint(df2['I'],df2['t']), df2['t'])[:-1], cumulative_trapezoid(df2['I'],df2['t']), rtol=1e-0)
    True
    '''
    # (equidistant) time step
    delta = t[1] - t[0]
...
def semi_integration(y, q=-0.5, delta_x=1, c1=8, c2=2):
    ...
    >>> np.allclose(semi_integration(semi_integration(df2['I'],delta_x=delta_x),delta_x=delta_x), cumulative_trapezoid(df2['I'],initial=0), rtol=1e-0)
    True
    """
